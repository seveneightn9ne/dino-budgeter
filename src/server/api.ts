...
export async function handle_init_get(
  request: ApiRequest<typeof Initialize>,
  actor: User,
): Promise<Partial<InitState>> {
  ...
  return db.tx(async (t) => {
    ...
    if (fields.has("frame")) {
      resData.frame = await frames.getOrCreateFrame(
        await gid(),
        request.index,
        t,
      );
    ...
    if (fields.has("transactions")) {
      resData.transactions = await transactions.getTransactions(
        request.index,
        await gid(),
        t,
      );
    ...
    if (fields.has("history")) {
      resData.history = await categories.getHistory(
        await gid(),
        request.index,
        t,
      );
