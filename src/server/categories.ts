...
export function getSpending(id: CategoryId, frame: FrameIndex, t: pgPromise.ITask<{}>): Promise<Money> {
    return t.manyOrNone("select amount from transactions where category = $1 and frame = $2 and alive = true",
        [id, frame]).then((rows) => {
            if (!rows) {
                return Money.Zero;
            }
...
export function getBudget(id: CategoryId, frame: FrameIndex, t: pgPromise.ITask<{}>): Promise<Money> {
    return t.oneOrNone("select budget from categories where id = $1 and frame = $2", [id, frame]).then((row) => {
...
export async function getCategories(gid: GroupId, frame: FrameIndex, t: pgPromise.ITask<{}>): Promise<Category[]> {
    const rows = await t.manyOrNone("select * from categories where gid = $1 and frame = $2 and alive = true",
        [gid, frame]);
...
export async function getHistory(gid: GroupId, frame: FrameIndex, t: pgPromise.ITask<{}>):
    ...
    const history: { [c: string]: Array<{ budget: Money, spending: Money }> } = {};
