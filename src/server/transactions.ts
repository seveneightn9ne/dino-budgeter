...
interface IDQuery { id: TransactionId }
...
function whereClause(query: Query) {
  return "id" in query
    ? "T.id = $1"
    : "frame" in query
    ? "T.gid = $1 and T.frame = $2 and T.alive = $3"
...
function vars(where: Query) {
  return "id" in where
    ? [where.id]
    : "frame" in where
    ? [where.gid, where.frame, where.alive]
    : /* settled */ [where.uid, where.settled, where.alive];
}
async function getTransactionsInner(
  where: Query,
  t: pgPromise.ITask<{}>,
): Promise<Transaction[]> {
...
export async function getOtherTid(
  tid: TransactionId,
  sid: SplitId,
  t: pgPromise.ITask<{}>,
): Promise<TransactionId> {
  const row = await t.one(
    "select tid from transaction_splits where sid = $1 and tid != $2",
    [sid, tid],
  );
  return row.tid;
}
...
export async function getUser(
  ...
  const row = await t.one(
    "select M.uid from transactions T left join membership M on T.gid = M.gid where T.id = $1",
    [tid],
  );
  return row.uid;
}

export async function getGid(
  tid: TransactionId,
  t: pgPromise.ITask<{}>,
): Promise<GroupId> {
...
export async function deleteTransaction(
  ...
  const linkedTxnRow = await t.oneOrNone(
    `select TS2.tid, M.uid, M2.uid as other_uid from transactions T
...
export async function getSid(
  ...
  const row = await t.oneOrNone(
    "select sid from transaction_splits where tid = $1",
    [tid],
  );
  return row ? row.sid : null;
}

export async function getBalanceFromDb(
  tid: TransactionId,
  t: pgPromise.ITask<{}>,
): Promise<Money> {
  const row = await t.one(
    `select T.amount, M.uid, T2.amount as other_amount, M2.uid as other_uid, S.payer
        from transactions T
        left join membership M on M.gid = T.gid
        left join transaction_splits TS on T.id = TS.tid
        left join shared_transactions S on TS.sid = S.id
