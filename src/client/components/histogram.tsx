...
export const Histogram: React.SFC<{
  ...
  const max = data.reduce(
    (m: Money, c: { budget: Money; spending: Money }): Money => {
  ...
  const grams = data.map(({ budget, spending }, i) => {
    ...
    return (
      <div
        ...
        key={`${budget.string()}-${spending.string()}-${monthName(i)}`}
      >
        <div className="histogram-bars">
          <div
            className="histogram-bar-total spending"
            style={{ height: height - 20 }}
          >
            <div
              className={`histogram-bar-filled ${color}`}
              style={{ height: `${heightPct(spending)}%` }}
            />
          </div>
          <div
            className="histogram-bar-total budget"
            style={{ height: height - 20 }}
          >
            <div
              className="histogram-bar-filled"
              style={{ height: `${heightPct(budget)}%` }}
            />
          </div>
        </div>
        <div className="histogram-label">
          <div className="histogram-label-month">{monthName(i)}</div>
  ...
  return <div className={`histogram ${className || ""}`}>{grams}</div>;
};
