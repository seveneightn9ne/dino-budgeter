...
export default class Friends extends React.Component<Props, State> {
  ...
  public closePayment = () => this.setState({ paymentOpen: false });
  public openCharge = () =>
    this.setState({
      chargeOpen: true,
      charge_youCharge: true,
  ...
  public closeCharge = () => this.setState({ chargeOpen: false });

  public onSubmitPayment = (f: UiFriend) => (e: React.FormEvent) =>
    this.onSubmitInner(
      e,
      f.email,
      f.uid,
      this.state.payment_amount,
      this.state.payment_youPay,
      this.state.payment_memo,
      true,
    )

  public onSubmitCharge = (f: UiFriend) => (e: React.FormEvent) =>
    this.onSubmitInner(
      e,
      f.email,
      f.uid,
      this.state.charge_amount,
      this.state.charge_youCharge,
      this.state.charge_memo,
      false,
    )

  public onSubmitInner = (
    e: React.FormEvent,
    email: string,
    uid: UserId,
    amountStr: string,
    ...
    const amount = new Money(amountStr);
    if (!amount.isValid()) {
      return;
    }
    util
  ...
  public allFriends = (props = this.props) => {
  ...
  public toggleCollapse = (uid: UserId) => () => {
    const newState = !this.state.collapsePayments[uid];
    this.setState({
      collapsePayments: { ...this.state.collapsePayments, [uid]: newState },
    });
  }

  public loadMore = (f: UiFriend) => () => {
    const payments = _.sortBy(
      this.props.debts[f.email].payments.filter(
        (p) => p.frame < this.state.latestFrame[f.uid],
      ),
      "frame",
    );
    ...
    this.setState({
      latestFrame: { ...this.state.latestFrame, [f.uid]: payments[0].frame },
    });
  }

  public displayName = (friend: UiFriend) => {
    return friend.name || friend.email;
  }

  public renderPayment = (f: UiFriend) => {
    return (
      <ControlledPoplet
        open={this.state.paymentOpen}
        onRequestOpen={this.openPayment}
        onRequestClose={this.closePayment}
        text={
          <span>
            <span className="fa-plus-circle fas"></span> Payment
          </span>
        }
      >
        <h2>New Payment</h2>
        <form onSubmit={this.onSubmitPayment(f)}>
          <label>
            ...
            <input
              ...
              onChange={util.cc(this, "payment_amount")}
            />
          </label>
          <label>
            Memo:{" "}
            <input
              type="text"
              value={this.state.payment_memo}
              onChange={util.cc(this, "payment_memo")}
            />
          </label>
          <div className="section" style={{ clear: "both" }}>
            <label className="nostyle">
              <input
                ...
                name="payer"
                value="0"
                checked={this.state.payment_youPay}
                onChange={(e) =>
                  this.setState({ payment_youPay: e.target.checked })
                }
              />{" "}
              You're paying {this.displayName(f)}
            </label>
            <label className="nostyle">
              <input
                type="radio"
              ...
              />{" "}
              {this.displayName(f)} is paying you
            </label>
          </div>
  ...
  public renderCharge = (f: UiFriend) => {
    return (
      <ControlledPoplet
        open={this.state.chargeOpen}
        onRequestOpen={this.openCharge}
        onRequestClose={this.closeCharge}
        text={
          <span>
            <span className="fa-plus-circle fas"></span> Charge
          </span>
        }
      >
        <h2>New Charge</h2>
        <form onSubmit={this.onSubmitCharge(f)}>
          <label>
            ...
            <input
              ...
              onChange={util.cc(this, "charge_amount")}
            />
          </label>
          <label>
            Memo:{" "}
            <input
              type="text"
              value={this.state.charge_memo}
              onChange={util.cc(this, "charge_memo")}
            />
          </label>
          <div className="section" style={{ clear: "both" }}>
            <label className="nostyle">
              <input
                ...
                name="payer"
                value="1"
                checked={this.state.charge_youCharge}
                onChange={(e) =>
                  this.setState({ charge_youCharge: e.target.checked })
                }
              />{" "}
              {this.displayName(f)} owes you
            </label>
            <label className="nostyle">
              <input
                type="radio"
  ...
  public render() {
    ...
    const rows = this.allFriends().map((f) => {
      ...
      const debt =
        f.debt.balance.cmp(Money.Zero) == 0 ? null : f.debt.balance.cmp(
            Money.Zero,
          ) > 0 ? (
      ...
      const payments = f.debt.payments
        ...
        .map((payment) => {
          ...
          return (
            <tr
              key={
                payment.date.toString() + payment.memo + payment.amount.string()
              }
            >
              <td>{text}</td>
              <td>{payment.memo}</td>
      ...
      const collapsePaymentButton = this.state.collapsePayments[f.uid] ? (
        <div
          className="friend-collapse fa-chevron-right fas"
          onClick={this.toggleCollapse(f.uid)}
        ></div>
      ) : (
        <div
          className="friend-collapse fa-chevron-down fas"
          onClick={this.toggleCollapse(f.uid)}
        ></div>
      );
      const paymentsTable = this.state.collapsePayments[
        f.uid
      ] ? null : payments.length == 0 ? null : (
      ...
      const paymentsLoadMore = this.state.collapsePayments[
        f.uid
      ] ? null : payments.length ==
        this.props.debts[f.email].payments.length ? null : (
        <div className="friend-more clickable" onClick={this.loadMore(f)}>
          Load older payments
        </div>
      );
      return (
        <div key={f.email}>
          <div className="friend-row">
            ...
            <div className="friend-actions">
              <div className="friend-pay">{payment}</div>
              <div className="friend-charge">{charge}</div>
            </div>
          </div>
          {paymentsTable}
          {paymentsLoadMore}
    ...
    return <div className="friends">{rows}</div>;
  }
}
