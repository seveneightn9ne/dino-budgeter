...
const CategoryPage: React.SFC<Props> = (props) => {
  ...
  const category = props.frame.categories.filter(
  ...
  if (props.match.params.name !== category.name) {
    ...
    return (
      <Redirect
        to={`/app/${month}/${year}/categories/${id}/${category.name}`}
      />
    );
  }
    ...
    <div className="category-page">
      <h2>
        ...
        <ClickToEditText
          ...
          onChange={(newName) =>
            props.onChangeCategory({ ...category, name: newName })
          }
          postData={{
            id: category.id,
      ...
      <h3>
        Spending this month: {spending.formatted()} /{" "}
        <ClickToEditMoney
          size={6}
          api={CategoryBudget}
      ...
      <MobileQuery
        desktop={histogramWithSize(props, 6)}
        mobile={histogramWithSize(props, 3)}
      />

      {props.transactions.length === 0 ? (
        <h3>There are no transactions this month.</h3>
      ) : (
        <div>
          <h3>Transactions</h3>
          <Transactions
...
const histogramWithSize = (props: Props, s: number) => (
  <Histogram
...
const histogramData = (
  h: Array<{ budget: Money; spending: Money }>,
  s: number,
) => {
...
const onUpdateBudget = (
  ...
  newCategory.balance = categories.updateBalanceWithBudget(category, newBudget);
