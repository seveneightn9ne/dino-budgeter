      ...
      >
        ...
        <form onSubmit={this.handleSubmit.bind(this)}>
          <label>
            Total:{" "}
            <input
              className={cls(this.state.totalErr)}
              type="text"
              size={6}
              onFocus={this.selectOnFocus}
              value={this.state.total}
              onChange={cc(this, "total")}
            />
          </label>
          <label className="first half">
            Your share:{" "}
            <input
              className={cls(this.state.yourErr)}
              type="text"
              size={1}
              onFocus={this.selectOnFocus}
              value={this.state.yourShare}
              onChange={cc(this, "yourShare")}
            />
          </label>
          <label className="half">
            Their share:{" "}
            <input
              className={cls(this.state.theirErr)}
              type="text"
              size={1}
              onFocus={this.selectOnFocus}
              value={this.state.theirShare}
              onChange={cc(this, "theirShare")}
            />
          </label>
          <div className="section" style={{ clear: "both" }}>
            <label className="nostyle">
              <input
                ...
                onChange={(e) => this.setState({ youPaid: e.target.checked })}
              />{" "}
              You paid
            </label>
            <label className="nostyle">
          ...
          <div className="section">
            You spent {youPay(this.state).formatted()}.
          </div>
...
function youPay(state: State): Money {
  return transactions.youPay(
    new Share(state.yourShare),
    new Share(state.theirShare),
    new Money(state.total),
  );
}
